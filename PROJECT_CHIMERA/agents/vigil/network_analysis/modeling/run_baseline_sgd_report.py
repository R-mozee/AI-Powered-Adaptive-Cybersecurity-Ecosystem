from __future__ import annotations

from pathlib import Path
import json
from collections import Counter

import numpy as np
import joblib

from sklearn.linear_model import SGDClassifier
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import (
    f1_score,
    confusion_matrix,
    classification_report,
)
from sklearn.utils.class_weight import compute_class_weight

from evaluate_utils import make_eval_report, save_eval_report


LABELS = [
    "BENIGN","DDoS","DoS","PortScan","BruteForce","WebAttack","Botnet","Infiltration","Heartbleed"
]


def load_npz_X(path: Path) -> np.ndarray:
    X = np.load(path, allow_pickle=False)["X"]
    if X.dtype != np.float32:
        X = X.astype(np.float32, copy=False)
    return X


def save_json(path: Path, obj) -> None:
    path.write_text(json.dumps(obj, indent=2), encoding="utf-8")


def main():
    FEATURES_DIR = Path(
        r"C:\Users\naren\Downloads\PROJECTS\PROJECT_CHIMERA\agents\vigil\network_analysis\datasets\processed\features\cicids2017_v1_covsplit"
    )

    X_train = load_npz_X(FEATURES_DIR / "X_train.npz")
    y_train = np.load(FEATURES_DIR / "y_train.npy", allow_pickle=True).astype(str)

    X_val = load_npz_X(FEATURES_DIR / "X_val.npz")
    y_val = np.load(FEATURES_DIR / "y_val.npy", allow_pickle=True).astype(str)

    X_test = load_npz_X(FEATURES_DIR / "X_test.npz")
    y_test = np.load(FEATURES_DIR / "y_test.npy", allow_pickle=True).astype(str)

    # class weights
    classes = np.array(LABELS, dtype=object)
    cw = compute_class_weight(class_weight="balanced", classes=classes, y=y_train)
    class_weight = {cls: float(w) for cls, w in zip(classes, cw)}

    pipe = Pipeline(steps=[
        ("scaler", StandardScaler(with_mean=False)),
        ("clf", SGDClassifier(
            loss="hinge",
            alpha=1e-5,
            penalty="l2",
            max_iter=4000,
            tol=1e-3,
            n_jobs=-1,
            class_weight=class_weight,
            random_state=42,
        ))
    ])

    pipe.fit(X_train, y_train)

    y_pred_val = pipe.predict(X_val)
    y_pred_test = pipe.predict(X_test)

    val_macro = float(f1_score(y_val, y_pred_val, average="macro", zero_division=0))
    test_macro = float(f1_score(y_test, y_pred_test, average="macro", zero_division=0))

    print("\n✅ SGD-SVM baseline + report complete.")
    print("VAL macro-F1:", val_macro)
    print("TEST macro-F1:", test_macro)

    # Reports
    report_text = classification_report(y_test, y_pred_test, labels=LABELS, zero_division=0, digits=4)
    print("\nPer-class report (TEST):\n", report_text)

    cm = confusion_matrix(y_test, y_pred_test, labels=LABELS)
    cm_norm = (cm / np.maximum(cm.sum(axis=1, keepdims=True), 1)).tolist()

    report = make_eval_report(y_test, y_pred_test, labels=LABELS)

    OUT_DIR = FEATURES_DIR / "baseline_sgd_svm"
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    # Save model
    joblib.dump(pipe, OUT_DIR / "model.joblib")
    save_eval_report(report, OUT_DIR / "eval_report.json")

    # Save confusion matrices + classification_report
    save_json(OUT_DIR / "confusion_matrix.json", {"labels": LABELS, "cm": cm.tolist()})
    save_json(OUT_DIR / "confusion_matrix_normalized.json", {"labels": LABELS, "cm_norm": cm_norm})
    (OUT_DIR / "classification_report.txt").write_text(report_text, encoding="utf-8")

    # Optional: top features per class (interpretable)
    # This assumes you have feature_names saved from Step 5.
    # If you saved "feature_names.json" in your features folder, we will use it.
    feat_names_path = FEATURES_DIR / "feature_names.json"
    if feat_names_path.exists():
        feature_names = json.loads(feat_names_path.read_text(encoding="utf-8"))
        clf: SGDClassifier = pipe.named_steps["clf"]
        W = clf.coef_  # shape: (n_classes, n_features) for multiclass
        top = {}
        for i, cls in enumerate(clf.classes_):
            w = W[i]
            idx_pos = np.argsort(w)[-15:][::-1]
            idx_neg = np.argsort(w)[:15]
            top[cls] = {
                "top_positive": [(feature_names[j], float(w[j])) for j in idx_pos],
                "top_negative": [(feature_names[j], float(w[j])) for j in idx_neg],
            }
        save_json(OUT_DIR / "top_features.json", top)

    summary = {
        "val_macro_f1": val_macro,
        "test_macro_f1": report.macro_f1,
        "test_weighted_f1": report.weighted_f1,
        "test_label_counts": dict(Counter(y_test)),
        "pred_label_counts": dict(Counter(y_pred_test)),
        "class_weight": class_weight,
    }
    save_json(OUT_DIR / "summary.json", summary)

    print("Artifacts:", OUT_DIR)


if __name__ == "__main__":
    main()
